...
f = @(x) tanh(alpha*x);
f_inv = @(x) atanh(x)/alpha; % The inverse of tanh is arctanh

% For numerical stability, limit the input to f_inv to avoid extreme values
f_inv = @(x) atanh(max(min(x, 0.99), -0.99))/alpha;
